...
class CdkStack(Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        # Acm for Cloudfront
        clfcert = acm.Certificate.from_certificate_arn(self, "cloudfrontdomainCert", reactconf["certificate_arn"])
        ...
        httplist.add_targets(
              port=80,
              targets= [
                  appfargateService.load_balancer_target(container_name="nginx", container_port=80, protocol=ecs.Protocol.HTTP)
                ]
            )
